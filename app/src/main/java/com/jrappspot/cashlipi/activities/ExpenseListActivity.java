...
public class ExpenseListActivity extends BaseActivity {
    ...
    @Override protected void onCreate(Bundle s){
        ...
        TextView tvHeader=findViewById(R.id.tvListHeader);if(tvHeader!=null)tvHeader.setText(" ব্যয়ের তালিকা");
        View header=findViewById(R.id.listHeader);if(header!=null)header.setBackground(getResources().getDrawable(R.drawable.bg_header_expense));
        setupFilterChips();
        viewController.attachControls(this, findViewById(android.R.id.content), this::applyFilter);
        ImageView ivClearE = findViewById(R.id.ivClearSearch);
        etSearch.addTextChangedListener(new TextWatcher(){public void beforeTextChanged(CharSequence s,int a,int b,int c){}public void onTextChanged(CharSequence s,int a,int b,int c){if(ivClearE!=null)ivClearE.setVisibility(s.length()>0?View.VISIBLE:View.GONE);applyFilter();}public void afterTextChanged(Editable s){}});
        if(ivClearE!=null)ivClearE.setOnClickListener(v->{etSearch.setText("");etSearch.requestFocus();});
        findViewById(R.id.btnAddNew).setOnClickListener(v-> {
            android.content.Intent i = new android.content.Intent(this, AddTransactionActivity.class);
    ...
    private void loadData(){allList=new ArrayList<>(db.getExpenseList());applyFilter();}
    ...
    private void applyFilter(){
        ...
        if(filteredList.isEmpty()){
            ...
            View tableContainer = findViewById(R.id.tableContainer);
            ...
            viewController.render(this, root, filteredList, "expense",
                    (item,pos) -> TransactionSheetHelper.showTransactionSheet(this, db, "expense", item, this::loadData),
                    (item,pos) -> TransactionSheetHelper.showTransactionSheet(this, db, "expense", item, this::loadData));
