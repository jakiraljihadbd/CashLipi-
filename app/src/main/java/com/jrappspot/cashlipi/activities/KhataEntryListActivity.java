...
public class KhataEntryListActivity extends BaseActivity {
    private DatabaseManager db;
    private RecyclerView rv;
    private LinearLayout emptyState;
    private EditText etSearch;
    private TextView tvTotal, tvCount, tvDenaTotal, tvPabonaTotal;
    private List<KhataEntry> allList=new ArrayList<>(), filteredList=new ArrayList<>();
    ...
    @Override protected void onCreate(Bundle s){
        super.onCreate(s);setContentView(R.layout.activity_khata_entry_list);
        ...
        findViewById(R.id.btnAddNew).setOnClickListener(v->
            startActivity(new android.content.Intent(this, AddKhataEntryActivity.class)));
        loadData();
    }
    @Override protected void onResume(){super.onResume();loadData();}
    private void loadData(){allList=new ArrayList<>(db.getKhataEntryList());
        if(tvDenaTotal!=null)tvDenaTotal.setText(DatabaseManager.formatAmount(db.getTotalKhataBaki()));
        if(tvPabonaTotal!=null)tvPabonaTotal.setText(DatabaseManager.formatAmount(db.getTotalKhataJoma()));
        applyFilter();}
    ...
    private void applyFilter(){
        ...
        for(KhataEntry e:allList){
            if(!q.isEmpty()&&!e.getCustomerName().toLowerCase().contains(q)&&!e.getNote().toLowerCase().contains(q))continue;
            if("baki".equals(typeFilter)&&!e.isBaki())continue;
            if("joma".equals(typeFilter)&&!e.isJoma())continue;
        ...
        double total=0;for(KhataEntry e:filteredList)total+=e.getAmount();
        ...
        if(filteredList.isEmpty()){rv.setVisibility(View.GONE);emptyState.setVisibility(View.VISIBLE);}
            ...
            rv.setAdapter(new KhataEntryListAdapter(this,filteredList,(item,pos)->{
                KhataEntrySheetHelper.showKhataEntrySheet(this, db, item, this::loadData);
            }));
